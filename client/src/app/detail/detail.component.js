...
var DetailComponent = (function () {
    function DetailComponent(http) {
        ...
        this.latitude = new Array();
        this.longitude = new Array();
    ...
    DetailComponent.prototype.getCoord = function () {
        var _this = this;
        for (var _i = 0, _a = this.voyageDay; _i < _a.length; _i++) {
            var day = _a[_i];
            for (var _b = 0, _c = day.schedule; _b < _c.length; _b++) {
                var schedule = _c[_b];
                for (var _d = 0, _e = schedule.activities; _d < _e.length; _d++) {
                    var activity = _e[_d];
                    this.http.get('https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyA4Ir550YaoKaIj9OiBlScsOWnPa9GT2So&address=' + activity.address)
                        ...
                        _this.latitude.push(data.json().results[0].geometry.location.lat);
                        _this.longitude.push(data.json().results[0].geometry.location.lng);
                    });
                }
            }
        }
